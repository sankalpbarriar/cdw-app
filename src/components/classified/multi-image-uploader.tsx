...
const DragAndDropContext = dynamic(() => import("./drag-and-drop-context").then((mod) => mod.DragAndDropContext),
    {
        ssr: false,
...
interface MultiImageUploader extends React.InputHTMLAttributes<HTMLInputElement> {
    className?: string,
}
...
export const MultiImageUploader = (props: MultiImageUploader) => {
    const { className } = props;

    const form = useFormContext<UpdateClassifedType>();

    //this allows to effectively manage states of a list of items in form
    ...
    const [progress, setProgress] = useState<ImageProrgress[]>([]);
    const [isUploading, setIsUploading] = useState(false);

    const handleItemProgress = useCallback((updates: ImageProrgress) => {
        setProgress((prev) => {
            const index = prev.findIndex((item) => item.uuid === updates.uuid);
            if (index === -1) return [...prev, updates];  //has'nt found index
            const newProgress = [...prev];   //otherwise build new progress
            newProgress[index] = { ...newProgress[index], ...updates }
            return newProgress;
        })
    }, []);

    const handleItemsUpdate = useCallback((newItems: ClassifiedImages) => {
        replace(newItems);  //also updating in form
        setItems(newItems);
    }, [replace]);
    ...
    const setFiles = useCallback(async (validFiles: File[]) => {
        ...
        for (const file of files) {
            ...
            uploader.onProgress((progress: ProgressArgs) => {
