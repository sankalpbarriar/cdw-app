...
export async function seedTaxonomy(prisma: PrismaClient) {
  ...
  async function insertInBatches<TUpsertArgs>(
    items: TUpsertArgs[],
    batchSize: number,
    insertFucntion: (batch: TUpsertArgs[]) => void
  ) {
    for (let i = 0; i < items.length; i += batchSize) {
      const batch = items.slice(i, i + batchSize);
      await insertFucntion(batch);
    }
  }
  ...
  await insertInBatches<Prisma.Prisma__ModelClient<unknown, unknown>>(
    ...
    BATCH_SIZE,
    async (batch) => {
      const model = await Promise.all(batch);
      console.log(`Seeded batch of length ${batch.length} models🌱`);
    }
  );
  ...
  await insertInBatches<Prisma.Prisma__ModelVariantClient<unknown, unknown>>(
    ...
    async (batch) => {
      ...
      console.log(`Seeded batch of length ${variants.length} models🌱`);
    }
  );
}
